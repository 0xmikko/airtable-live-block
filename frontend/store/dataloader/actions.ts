...
export const createDataLoaderListActions = (
  ...
  return (
    ...
    if (action === undefined || action.error) {
      dispatch(
        updateStatus(hash || '0', STATUS.FAILURE, action === undefined ? "Network error" : action.payload.message),
      );
    } else {
...
export const createDataLoaderDetailActions = (
  ...
  if (action === undefined || action.error) {
    dispatch(updateStatus(hash || '0', STATUS.FAILURE, action === undefined ? "Network error" : action.payload.message));
  } else {
    dispatch(updateStatus(hash || '0', STATUS.SUCCESS));
  }
  return action;
